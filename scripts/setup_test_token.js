// scripts/setup_test_token.js
// Run with: node scripts/setup_test_token.js
// Creates a staff profile for the logged-in user AND generates a test token
...
async function main() {
    ...
    if (!patients || patients.length === 0) {
        console.log('\nNo waiting patients found. Looking for any patient...');
        ...
        if (!anyPatients || anyPatients.length === 0) {
            console.log('No patients exist in database! Create one via the Intake form first.');
            return;
        }
        patients.push(...anyPatients);
    }
